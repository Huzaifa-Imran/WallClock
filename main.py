...
def setup():
    ...
    for i in range(rowSize):
        ...
        for j in range(colSize):
            rowClock.append(Clock((j*2+1) * radius, (i*2+1) * radius, radius, 1))
...
def setPattern(pattern):
    ...
    if pattern == 0:
        ...
        for i in range(len(clock)):
            for j in range(len(clock[i])):
                clock[i][j].reinitiate(j*distance + BOT, j*distance + TOP, 0, 0, True)
    ...
    if pattern == 1:
        for i in range(len(clock)):
            ...
            for j in range(len(clock[i])):
                k = (j+i) % 2
                clock[i][j].reinitiate(k*BOT + baseAngle, (k+1)*BOT + baseAngle, 1, 0, True)
        ...
        for i in range(len(clock)):
            ...
            for j in range(len(clock[i])):
                k = (j+i) % 2
                clock[i][j].reinitiate(k*BOT + baseAngle, (k+1)*BOT + baseAngle, (i+j)%2, (i+j)%2, True)
...
digitMaker = [createZero, createOne, createTwo, createThree, createFour, createFive, createSix, createSeven, createEight, createNine]
